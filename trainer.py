...
def parse_args():
    ...
    parser.add_argument('--style', type=str, default='', help='distill or not')
    parser.add_argument('--associate_modal', type=str, default='V+T', choices=['T','V','V+T','VT'], help='choose which modal for association')
    parser.add_argument('--support_set_number_V', type=int, default=8, help='use how many support videos for training')
    parser.add_argument('--support_set_number_T', type=int, default=8, help='use how many GT captions')
    ...
    parser.add_argument('--alpha_V', type=float, default=0.2, help='loss balance parameter')
    parser.add_argument('--alpha_T', type=float, default=0.2, help='loss balance parameter')
    parser.add_argument('--alpha_V_T', type=float, default=0.2, help='loss balance parameter')
    parser.add_argument('--beta_V', type=float, default=1, help='loss balance parameter')
    parser.add_argument('--beta_T', type=float, default=1, help='loss balance parameter')
    parser.add_argument('--beta_V_T', type=float, default=1, help='loss balance parameter')
    parser.add_argument('--gamma_V', type=float, default=1, help='loss balance parameter')
    parser.add_argument('--gamma_T', type=float, default=1, help='loss balance parameter')
    ...
    parser.add_argument('--cv_name', type=str, default='reconstruct-association-separatetraining', help='')
    #tag
...
def main():
    ...
    ssn = 'ssn_V_%s_ssn_T_%s' % (opt.support_set_number_V, opt.support_set_number_T)

    if opt.associate_modal == 'VT':
        distill_info = 'alpha_V_%s_alpha_T_%s_alpha_V_T_%s_beta_V_%s_beta_T_%s_beta_V_T_%s_gamma_V_%s_gamma_T_%s' % \
                        (opt.alpha_V, opt.alpha_T, opt.alpha_V_T, opt.beta_V, opt.beta_T, opt.beta_V_T, opt.gamma_V, opt.gamma_T)
    else:
        distill_info = 'alpha_V_%s_alpha_T_%s_beta_V_%s_beta_T_%s_gamma_V_%s_gamma_T_%s' % \
                        (opt.alpha_V, opt.alpha_T, opt.beta_V, opt.beta_T, opt.gamma_V, opt.gamma_T)
    ...
    opt.logger_name = os.path.join(rootpath, collections_pathname['train'], opt.cv_name, collections_pathname['val'], model_info, text_encode_info,
                            visual_encode_info, mapping_info, loss_info, optimizer_info, opt.associate_modal, ssn, distill_info, opt.postfix)
    ...
    data_loaders = data.get_train_data_loaders(
        caption_files, visual_feats, tag_path, tag_vocab_path, rnn_vocab, bow2vec, opt.style, opt.associate_modal, opt.support_set_number_V, opt.support_set_number_T, opt.batch_size, opt.workers, video2frames)
    val_video_ids_list = data.read_video_ids(caption_files['val'])
    val_vid_data_loader = data.get_vis_data_loader(visual_feats['val'], opt.support_set_number_V, opt.batch_size, opt.workers, video2frames['val'], video_ids=val_video_ids_list)
    val_text_data_loader = data.get_txt_data_loader(caption_files['val'], rnn_vocab, bow2vec, opt.support_set_number_T, opt.batch_size, opt.workers, video2frames['val'], visual_feats['val'])
    
    # Construct the model
    model = get_model(opt.model)(opt)
    opt.we_parameter = None
    
    if opt.style == 'distill':
        if 'V' in opt.associate_modal:
            if os.path.exists(os.path.join(opt.logger_name,'teacher_V.pth.tar')):
                opt.stop_teacher_V = True
                logging.info('loading best teacher_V model')
                model.load_state_dict_teacher_V(torch.load(os.path.join(opt.logger_name,'teacher_V.pth.tar')))       
            else:
                opt.stop_teacher_V = False
            val_vid_data_loader_support = data.get_vis_data_loader_support(visual_feats['val'], opt.support_set_number_V, opt.batch_size, opt.workers, video2frames['val'], video_ids=val_video_ids_list)
            # teacher_V_rsum = validate.validate_V(opt, tb_logger, val_vid_data_loader_support, val_text_data_loader, model, measure=opt.measure)
        if 'T' in opt.associate_modal:
            if os.path.exists(os.path.join(opt.logger_name,'teacher_T.pth.tar')):
                opt.stop_teacher_T = True
                logging.info('loading best teacher_T model')
                model.load_state_dict_teacher_T(torch.load(os.path.join(opt.logger_name,'teacher_T.pth.tar')))
            else:
                opt.stop_teacher_T = False
            val_text_data_loader_support = data.get_txt_data_loader_support(caption_files['val'], rnn_vocab, bow2vec, opt.support_set_number_T, opt.batch_size, opt.workers, video2frames['val'], visual_feats['val'])
        if opt.associate_modal == 'VT':
            opt.stop_teacher_V_T = False
    ...
    for epoch in range(opt.num_epochs):
        ...
        if opt.style == 'distill':
            # if opt.associate_modal == 'VT' and not opt.stop_teacher_V_T:
            #     teacher_V_rsum = validate.validate_V(opt, tb_logger, val_vid_data_loader_support, val_text_data_loader, model, measure=opt.measure)
            #     logging.info(' * Teacher_V: Current perf: {}'.format(teacher_V_rsum))
            #     teacher_T_rsum = validate.validate_T(opt, tb_logger, val_vid_data_loader, val_text_data_loader_support, model, measure=opt.measure)
            #     logging.info(' * Teacher_T: Current perf: {}'.format(teacher_T_rsum))
            #     continue
            if 'V' in opt.associate_modal and not opt.stop_teacher_V:
                teacher_V_rsum = validate.validate_V(opt, tb_logger, val_vid_data_loader_support, val_text_data_loader, model, measure=opt.measure)
                logging.info(' * Teacher_V: Current perf: {}'.format(teacher_V_rsum))
                if teacher_V_rsum > best_rsum_V:
                    best_rsum_V = teacher_V_rsum
                    torch.save(model.state_dict_teacher_V(), os.path.join(opt.logger_name,'teacher_V.pth.tar'))
                else:
                    no_impr_counter_V += 1
                    if no_impr_counter_V > 4:
                        opt.stop_teacher_V = True
                        logging.info('stop training teacher_V')
                        logging.info('-'*20)
                        logging.info('loading best teacher_V model')
                        model.load_state_dict_teacher_V(torch.load(os.path.join(opt.logger_name,'teacher_V.pth.tar')))
                        teacher_V_rsum = validate.validate_V(opt, tb_logger, val_vid_data_loader_support, val_text_data_loader, model, measure=opt.measure)
                        logging.info(' * best Teacher_V perf: {}'.format(teacher_V_rsum))
                        logging.info('-'*20)
            if 'T' in opt.associate_modal and not opt.stop_teacher_T:
                teacher_T_rsum = validate.validate_T(opt, tb_logger, val_vid_data_loader, val_text_data_loader_support, model, measure=opt.measure)
                logging.info(' * Teacher_T: Current perf: {}'.format(teacher_T_rsum))
                if teacher_T_rsum > best_rsum_T:
                    best_rsum_T = teacher_T_rsum
                    torch.save(model.state_dict_teacher_T(), os.path.join(opt.logger_name,'teacher_T.pth.tar'))
                else:
                    no_impr_counter_T += 1
                    if no_impr_counter_T > 2:
                        opt.stop_teacher_T = True
                        logging.info('stop training teacher_T')
                        logging.info('-'*20)
                        logging.info('loading best teacher_T model')
                        model.load_state_dict_teacher_T(torch.load(os.path.join(opt.logger_name,'teacher_T.pth.tar')))
                        teacher_T_rsum = validate.validate_T(opt, tb_logger, val_vid_data_loader, val_text_data_loader_support, model, measure=opt.measure)
                        logging.info(' * best Teacher_T perf: {}'.format(teacher_T_rsum))
                        logging.info('-'*20)
                        if 'V' in opt.associate_modal:
                            opt.stop_teacher_V = True
                            logging.info('stop training teacher_V')
...
def train(opt, train_loader, model, epoch):
    ...
    for i, train_data in enumerate(train_loader):
        ...
        # Update the model
        if opt.style == 'distill':
            if opt.associate_modal == 'T':
                b_size, loss_student, loss_T_triplet, loss_T_cap, loss_T_vid, loss_T_sim = model.train_emb(i, epoch, opt, *train_data)
                progbar.add(b_size, values=[('Triplet-s', loss_student),('Triplet-T', loss_T_triplet),('T-cap', loss_T_cap),('T-vid', loss_T_vid),('T-sim', loss_T_sim)])
            elif opt.associate_modal == 'V':
                b_size, loss_student, loss_V_triplet, loss_V_cap, loss_V_vid, loss_V_sim = model.train_emb(i, epoch, opt, *train_data)
                progbar.add(b_size, values=[('Triplet-s', loss_student),('Triplet-V', loss_V_triplet),('V-cap', loss_V_cap),('V-vid', loss_V_vid),('V-sim', loss_V_sim)])
            elif opt.associate_modal == 'V+T':
                b_size, loss_student, loss_V_triplet, loss_V_cap, loss_V_vid, loss_V_sim, loss_T_triplet, loss_T_cap, loss_T_vid, loss_T_sim = model.train_emb(i, epoch, opt, *train_data)
                progbar.add(b_size, values=[('Triplet-s', loss_student),('Triplet-V', loss_V_triplet),('V-cap', loss_V_cap),('V-vid', loss_V_vid),('V-sim', loss_V_sim),('Triplet-T', loss_T_triplet),('T-cap', loss_T_cap),('T-vid', loss_T_vid),('T-sim', loss_T_sim)])
            elif opt.associate_modal == 'VT':
                b_size, loss_student, loss_V_triplet, loss_V_cap, loss_V_vid, loss_V_sim, loss_T_triplet, loss_T_cap, loss_T_vid, loss_T_sim, loss_V_T_cap, loss_V_T_vid, loss_V_T_sim = model.train_emb(i, epoch, opt, *train_data)
                progbar.add(b_size, values=[('Triplet-s', loss_student),('Triplet-V', loss_V_triplet),('V-cap', loss_V_cap),('V-vid', loss_V_vid),('V-sim', loss_V_sim),('Triplet-T', loss_T_triplet),('T-cap', loss_T_cap),('T-vid', loss_T_vid),('T-sim', loss_T_sim),('V_T-cap', loss_T_cap),('V_T-vid', loss_T_vid),('V_T-sim', loss_T_sim)])
        else:
            b_size, loss_student = model.train_emb(i, epoch, opt, *train_data)
            progbar.add(b_size, values=[('Triplet-s', loss_student)])
    ...
    if opt.style == 'distill':
        if opt.associate_modal == 'T':
            logging.info('Triplet-s: %s, Triplet-T: %s, T-cap: %s, T-vid: %s, T-sim: %s' % (loss_student, loss_T_triplet, loss_T_cap, loss_T_vid, loss_T_sim))
        elif opt.associate_modal == 'V':
            logging.info('Triplet-s: %s, Triplet-V: %s, V-cap: %s, V-vid: %s, V-sim: %s' % (loss_student, loss_V_triplet, loss_V_cap, loss_V_vid, loss_V_sim))
        elif opt.associate_modal == 'V+T':
            logging.info('Triplet-s: %s, Triplet-V: %s, V-cap: %s, V-vid: %s, V-sim: %s, Triplet-T: %s, T-cap: %s, T-vid: %s, T-sim: %s' % (loss_student, loss_V_triplet, loss_V_cap, loss_V_vid, loss_V_sim, loss_T_triplet, loss_T_cap, loss_T_vid, loss_T_sim))
        elif opt.associate_modal == 'VT':
            logging.info('Triplet-s: %s, Triplet-V: %s, V-cap: %s, V-vid: %s, V-sim: %s, Triplet-T: %s, T-cap: %s, T-vid: %s, T-sim: %s, V_T-cap: %s, V_T-vid: %s, V_T-sim: %s' % (loss_student, loss_V_triplet, loss_V_cap, loss_V_vid, loss_V_sim, loss_T_triplet, loss_T_cap, loss_T_vid, loss_T_sim, loss_V_T_cap, loss_V_T_vid, loss_V_T_sim))
    else:
        logging.info('Triplet-s: %s' % (loss_student))
